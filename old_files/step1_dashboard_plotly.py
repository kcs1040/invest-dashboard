...
#   * Force numeric y (avoid 0..N categorical axis bug)
# -----------------------------------------------------------
...
def eom(x):
    """End-of-month sampler (returns numeric pd.Series with DatetimeIndex)."""
    if x is None:
        return None
...
def num_series(s: pd.Series) -> pd.Series:
    """Ensure numeric dtype with datetime index, drop bad values."""
    if s is None:
        return None
...
def to_usd_terms(series_local, fx_series, quote_type):
    ...
    if quote_type == "usd_per_local":
        ...
        usd = s_loc
    usd = pd.to_numeric(usd, errors="coerce").dropna()  # ensure numeric
    return usd
...
def fig_line(ts, name, yaxis_title, unit=None, show_range_slider=True, subtitle=None, log_y=False):
    ...
    fig.update_yaxes(type="log" if log_y else "linear")  # force numeric axis
...
def fig_two_lines(ts1, ts2, name1, name2, yaxis_title, unit=None,
    ...
    fig.update_yaxes(type="linear")  # force numeric
...
def main():
    ...
    for name, s_local in local_eom.items():
        ...
        if usd_series is None and meta["fx"] is None:
            usd_series = num_series(s_local)  # already USD
    ...
    for name, series in majors_eom.items():
        ...
        figs_cmaj.append((title, fig_line(series, title, "Price (USD, log)", unit="idx",
                                          subtitle=f"{title} — Major crypto", log_y=False)))
    ...
    for name, series in alts_eom.items():
        ...
        figs_calt.append((title, fig_line(series, title, "Price (USD, log)", unit="idx",
                                          subtitle=f"{title} — Altcoin", log_y=False)))

    # ===== HTML assemble with TABS =====
    from plotly.offline import plot as plot_offline

    def plot_div(fig, include_js=False):
        # Responsive config applied everywhere
    ...
    html_parts = [
        ...
        ".grid > div { min-width: 0; }",  # prevent overflow issues
